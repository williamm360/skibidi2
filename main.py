...
class IntroScene(Scene):
    def construct(self):
        ...
        name_txt_v.arrange(DOWN, buff=0.2).scale(1.5).to_edge(
            LEFT)  # Added buff for consistent spacing

        full_name_array = [Tex(word) for word in ["Surface", "Adaptive",
                                                  "Normal-Oriented", "Gaussians", "For", "Bird's", "Eye", "View"]]

        descr_txt = Tex(
            "A Solution for monocular zero-shot BEV reconstruction").match_y(name_txt_h).shift(UP*0.5)
        paper_names = Tex("Presented by Ilian Djorf, Noah Favreau,\\\ Louis Turpault and William Ma").match_y(
            descr_txt).shift(DOWN*1.5)
        credit_txt = Tex("Animated with ManimCE").match_y(
            paper_names).shift(DOWN*1.5)
        ...
        application_outline = DashedVMobject(
            application_outline, num_dashes=60)
        application_outline.to_corner(DL).shift(DOWN*0.25).shift(LEFT*0.25)

        method_txt = Tex("Method").next_to(method_outline, RIGHT*1.2)
        application_txt = Tex("Application").next_to(
            application_outline, RIGHT*1.2)
        ...
        self.wait(1.5)
        self.play(FadeOut(descr_txt), FadeOut(paper_names),
                  FadeOut(credit_txt), run_time=1)
        self.wait(0.8)
        ...
        desc_comp = VGroup(
            ...
            Text("model regression ")).arrange(DOWN)
        ...
        video_normal = VideoMobject(
            r"/home/willm277/Desktop/manim_BEV/media/videos/main/1080p15/NormalOrientedHint.mp4").scale(0.9).to_edge(RIGHT)
        ...
        self.play(
            ...
            run_time=1)
        video_gaussian = VideoMobject(
            r"/home/willm277/Desktop/manim_BEV/media/videos/main/1080p15/SmoothGaussianTransition.mp4").scale(0.9).to_edge(RIGHT)
...
class SurfaceAdaptiveHint:
    ...
    def subscene_animation(scene):
        wood_texture = ImageMobject(
            "media/images/main/wood.jpeg").scale(0.08).to_edge(RIGHT)
        snow_texture = ImageMobject(
            "media/images/main/snow.jpg").scale(0.4256).next_to(wood_texture, UP*1.3)
        glass_texture = ImageMobject(
            "media/images/main/glass.jpg").scale(0.133).next_to(wood_texture, DOWN*1.3)
...
class NormalOrientedHint(ThreeDScene):
    ...
    def construct(self):
        ...
        normal_2d = Arrow3D(
            start=axes.c2p(1, 0, 1),
            end=axes.c2p(1, 0, 1) + normal_vec * 1.5,
            color=RED
        )
        ...
        self.normal_3d = Arrow3D(
            start=center,
            end=center + normal_vec * 1.5,
            color=RED
        )
        ...
        self.move_camera(
            ...
            theta=-230 * DEGREES,
            added_anims=[
                scene_group.animate.shift(UP * 3 + RIGHT * 2)
            ],
            run_time=2
        )
        formula = MathTex(
        ...
        angle_text = always_redraw(lambda: VGroup(
            MathTex(
                f"\\theta = {self.get_angle_between_normals():.1f}^\\circ"),
            MathTex(
                f"\\cos(\\theta) = {np.cos(self.get_angle_between_normals() * DEGREES):.3f}")
        ).arrange(DOWN).to_corner(UL).add_background_rectangle()
        )
...
class GaussiansHint(ThreeDScene):
    ...
    def construct(self):
        ...
        primary_arrow = Arrow3D(
            start=ORIGIN - primary_direction *
            np.sqrt(eigenvalues[max_variance_idx]) * 4,
...
class BEVHint:
    ...
    def create_scene(scene):
        ...
        map_txt = Text("BEV View", font_size=32).rotate(
            90*DEGREES).next_to(map_view, LEFT)
        image_txt = Text("Image", font_size=32).rotate(
            90*DEGREES).next_to(true_view, LEFT)
        ...
        scene.play(FadeIn(image_cabin),
                  map_view.animate.set_fill(color=GRAY_A,opacity=0.6))


        tree_rect_img = Rectangle(height=3, width=0.4, color=RED).set_z_index(4).next_to(
            image_cabin, RIGHT).shift(LEFT*0.8 + UP*0.1).set_fill(color=RED, opacity=0.6)
        house_rect_img = Rectangle(height=1.3, width=1.6, color=BLUE).set_z_index(2).next_to(
            tree_rect_img, LEFT).shift(LEFT*0.18 + UP * 0.5).set_fill(color=BLUE, opacity=0.6)
        path_rect_img = Rectangle(height=1.5, width=1.8, color=YELLOW).set_z_index(3).next_to(
            tree_rect_img, LEFT).shift(LEFT*0.2 + DOWN * 0.9).set_fill(color=YELLOW, opacity=0.6)
        forest_rect_img = Rectangle(height=1.5, width=1.6, color=PINK).set_z_index(1).next_to(
            image_cabin, RIGHT).shift(LEFT*1.9 + UP*0.8).set_fill(color=PINK, opacity=0.6)
        bush_rect_img = Rectangle(height=1.5, width=0.3, color=ORANGE).set_z_index(
            1).next_to(house_rect_img, LEFT).shift(LEFT*0.2 + UP*0.25).set_fill(color=ORANGE, opacity=0.6)
        
        forest_rect_bev = Rectangle(width=1.8, height=0.6, color=PINK).to_corner(UR).set_fill(color=PINK, opacity=0.6)
        tree_circle_bev = Circle(radius=0.1, color=RED).next_to(forest_rect_bev, DOWN).shift(
            DOWN*2).set_fill(color=RED, opacity=0.6)
        house_rect_bev = Rectangle(width=1.8, height=1.4, color=BLUE).rotate(-30*DEGREES).next_to(
            forest_rect_bev, DOWN).shift(LEFT*1+UP*0.7).set_fill(color=BLUE, opacity=0.6).set_z_index(10)
        path_rect_bev = Rectangle(width=1.2, height=1.6, color=YELLOW).rotate(
            20*DEGREES).next_to(tree_circle_bev, LEFT).shift(UP*0.5 + LEFT*0.2).set_fill(color=YELLOW, opacity=0.6)
        bush_circle_bev = Circle(radius=0.1, color=ORANGE).next_to(
            house_rect_bev, LEFT).shift(LEFT*0.3 + UP * 0.2).set_fill(color=ORANGE, opacity=0.6)
        clipped_image = Intersection(path_rect_bev, map_view, color=YELLOW).set_fill(
            color=YELLOW, opacity=0.6)
        

        scene.play(Create(tree_rect_img),
                Create(tree_circle_bev))

        scene.play(Create(house_rect_img),
                  Create(house_rect_bev))

        scene.play(Create(path_rect_img),
                  Create(clipped_image))
        
        scene.play(Create(forest_rect_img),
                  Create(forest_rect_bev))

        scene.play(Create(bush_rect_img),
                  Create(bush_circle_bev))
        scene.wait(2)

        scene.play(FadeOut(tree_circle_bev),
